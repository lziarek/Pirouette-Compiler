...
(* let dot_to_file filename dot_code =
  let file_oc = open_out (filename ^ ".dot") in
  ...
  if !outfile_name <> "" then(
    if !prettyprint_flag then (
      let file_oc = open_out (!outfile_name (*^ ".extension"*)) in
        ...
        close_out file_oc);
    (* if !dot_flag then dot_to_file !outfile_name (generate_dot_code program); *)
    ...
    (* if !dot_flag then print_endline (generate_dot_code program); *)
